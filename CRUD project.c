...
int main() {
	int running = TRUE;	// running은 while의 조건식으로 변수이므로 1이 아닌 true를 저장하는 것
	...
	while (running) {
		...
		if (loginSuccess == TRUE) {
			...
			case END:
				printf("\nLeave this program.\n");
				running = FALSE;	// running 변수는 while의 조건식에서 사용하므로 0을 저장한게 아니라 false를 저장한 것
				break;
